<template>
  <van-nav-bar title="道具合成" left-text="返回" left-arrow @click-left="returnHomepage" />
  ...
  <van-tabs v-model:active="active">
    <van-tab title="设计师证明">
      ...
      <van-cell-group inset title="目前持有">
        <template v-for="i in [0, 1, 2, 3]" :key="i">
          <van-cell v-if="i < aTarget" :title="aName[i]" center>
            <template #right-icon>
              <van-stepper v-model="a[i]" integer input-width="64px" min="0" @change="calculateA" />
            </template>
          </van-cell>
        </template>
      </van-cell-group>

      <van-cell-group inset title="需要合成">
    ...
    <van-tab title="工艺学习">
      <van-cell-group inset title="目前持有">
        <van-cell v-for="i in [0, 1]" :key="i" :title="bName[i]" center>
          <template #right-icon>
            <van-stepper v-model="b[i]" integer input-width="64px" min="0" @change="calculateB" />
          </template>
        </van-cell>
        <van-cell title="清空" can-click is-link @click="b = [0, 0]" />
      </van-cell-group>

      <van-cell-group inset title="需要合成">
        <van-cell v-for="i in [1, 2]" :key="i" :title="bName[i]" :value="bNeed[i]" />
      </van-cell-group>
    </van-tab>
  </van-tabs>

  <van-popup v-model:show="showPicker" round position="bottom">
    <van-picker :columns="aName.slice(2)" :default-index="2" @confirm="confirmA" @cancel="showPicker = false" />
  </van-popup>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      showPicker: false,
      a: [0, 0, 0, 0, 0],
      aName: ['设计助理证明', '初级设计师证明', '设计师证明', '资深设计师证明', '创意总监证明'],
      aNeed: [0, 0, 0, 0, 0],
      aTarget: 4,
    ...
    calculateA () {
      ...
      let maxD = (a, b, c) => {
        if (Math.floor((b + maxB(a)) / 2) < Math.floor(c / 3))
          ...
          return Math.floor((8 * c + 2 * b + a) / 28)
      }
      let maxE = (a, b, c, d) => {
        if (Math.floor((c + maxC(a, b)) / 2) < Math.floor(d / 3))
          ...
          return Math.round((a + 2 * b + 8 * c + 28 * d) / 100) // 摆烂了
      }
      ...
      if (this.aTarget == 4) {
        let flag = true
        let E = maxE(a, b, c, d), D, C, B
        while (flag) {
          if (3 * E > d + maxD(a, b, c)) { E -= 1; continue; }
          D = Math.max(3 * E - d, 0)
          if (3 * D + 2 * E > c + maxC(a, b)) { E -= 1; continue; }
          C = Math.max(3 * D + 2 * E - c, 0)
          if (3 * C + 2 * D > b + maxB(a)) { E -= 1; continue; }
          B = Math.max(3 * C + 2 * D - b, 0)
          if (2 * B + 2 * C > a) { E -= 1; continue; }
          flag = false
        }
